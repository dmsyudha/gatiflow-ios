...
public final class MobileHub {
    public static let shared = MobileHub()
    private init() {}
    ...
    public func start(appToken: String, services: [MobileHubService] = []) {
        let config = Config.Builder(appToken: appToken).build()
        start(config: config, services: services)
    }

    public func start(config: Config, services: [MobileHubService] = []) {
        self.config = config
